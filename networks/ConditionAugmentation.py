...
class ConditionAugmentor(th.nn.Module):
    ...
    def __init__(self, input_size, latent_size, use_eql=True, device=th.device("cpu")):
        ...
        # required modules:
        if use_eql:
            from .CustomLayers import _equalized_linear
    ...
    def forward(self, x, epsilon=1e-12):
        ...
        combined = self.transformer(x) #(batch_size, 512)

        # use the reparameterization trick
        mid_point = self.latent_size # 256
        ...
        #print(epsilon.shape, sigmas.shape, mus.shape)
