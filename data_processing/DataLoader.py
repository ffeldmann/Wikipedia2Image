...
class RawTextFace2TextDataset(Dataset):
    ...
    def __load_data(self):
        ...
        :return: images, descs => lists of images and descriptions
        """
        from data_processing.TextExtractor import read_annotations, basic_preprocess
        images, descs = read_annotations(self.data_file_path)
        # preprocess the descriptions:
        descs = basic_preprocess(descs)

        return images, descs

    def __init__(self, data_path, img_dir, img_transform=None):
        """
        constructor of the class
        :param data_path: path containing images and text folders
        :param img_dir: path to the images directory
        :param img_transform: torch_vision transform to apply
        """

        # create state:
        #self.base_path = img_dir #/images
    ...
    def __getitem__(self, ix):
        ...
        # read the image at the given index
        img_file_path = os.path.join(self.data_file_path + "/images", self.images[ix])
        img = PIL.Image.open(img_file_path)
        while img.mode != 'RGB':
            #print("Searching new image, image channels: ", print(img.mode))
            ix = ix+1 if ix<2000 else ix-1 # avoid indexing errors
            img_file_path = os.path.join(self.data_file_path + "/images", self.images[ix])
            img = PIL.Image.open(img_file_path)
...
def get_data_loader(dataset, batch_size, num_workers, mode = "training"):
    """
